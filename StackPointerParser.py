...
def main():
    ...
    args_parser.add_argument('--mode', choices=['RNN', 'LSTM', 'GRU', 'FastLSTM'], help='architecture of rnn', default='LSTM')
    ...
    args_parser.add_argument('--char_num_filters', type=int, default=128, help='Number of filters in CNN(Character Level)')
    ...
    args_parser.add_argument('--gamma', type=float, default=0.00005, help='weight for regularization')
    ...
    args_parser.add_argument('--p_rnn', nargs=2, type=float, default=[0.2, 0.2], help='dropout rate for RNN')
    args_parser.add_argument('--p_in', type=float, default=0.2, help='dropout rate for input embeddings')
    args_parser.add_argument('--p_out', type=float, default=0.2, help='dropout rate for output layer')
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        network.eval()
        tmp_root = '{}_tmp'.format(model_path)
        ...
        if dev_lcorrect_nopunc < dev_lcorr_nopunc or (dev_lcorrect_nopunc == dev_lcorr_nopunc and dev_ucorrect_nopunc < dev_ucorr_nopunc):
            ...
            pred_filename = '%s/%spred_test%d' % (tmp_root, str(uid), epoch)
            pred_writer.start(pred_filename)
            gold_filename = '%s/%sgold_test%d' % (tmp_root, str(uid), epoch)
            gold_writer.start(gold_filename)
